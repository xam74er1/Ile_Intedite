...
public class Controleur implements Observateur{
	...
	private void finDeTour() {
		...
		if (givePlayer.getListeCarteJoueur().size() > 5) {
			...
			ihm.addConsole("Vous avez " + (givePlayer.getListeCarteJoueur().size()-5) + " cartes en trop dans votre main, choisir les cartes ÃƒÂ  dÃƒÂ©fausser :");
		}
	...
	private void conditionDefaite() {
		// TODO - implement Controleur.conditionDÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â©faite
		throw new UnsupportedOperationException();
	}
	...
	public void afficherListeCarteJoueur() {
		...
		if (getJoueurTour().getListeCarteJoueur().size() > 5) {
			lastAction = TypeMessage.Defausse_Joueur;
			ihm.addConsole("Vous avez " + (getJoueurTour().getListeCarteJoueur().size()-5) + " cartes en trop dans votre main, choisir les cartes ÃƒÂ  dÃƒÂ©fausser :");
		}
			...
			for(Tuile t : grille.getTuilesListe().values()) {
				if(temple>-1 && (t.getNum()==341 || t.getNum()==342) && t.getStatue()==-2) {
					...
					if(temple==2||caverne==2||palais==2||jardin==2) {
						return -1;												//Deux cases de rÃƒÂ©cupÃƒÂ©ration de trÃƒÂ©sor coulÃƒÂ©es
					}
